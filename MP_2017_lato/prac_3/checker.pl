...
:- use_module(kamil_szubinski_tests).
:- use_module(mateusz_hazy).
...
show(Name) :-
    tests(Name, Input, Ans),
    ( validate_test(Name, Input, Ans) -> show_test(Input); format("invalid test~n")).
...
try_show(Path, Codes, Solution) :-
    ( catch(
        parse(Path, Codes, Solution),
        syntax_error(Message, Pos),
        Solution=syntax_error(Message, Pos)), !
    ; Solution=syntax_error
    ).
