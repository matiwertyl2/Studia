...
struct powerproduct
{
   ...
   static int compare( const powerproduct& c1, const powerproduct& c2 ) {
     if (c1.repr.size()==0) return -1;
     for (size_t i=0; i<c1.repr.size(); i++) {
       if (i>= c2.repr.size()) return 1;
       //std::cout << "a\n";
       if (c1.repr[i].v<c2.repr[i].v) return -1;
       else if (c2.repr[i].v>c2.repr[i].v) return 1;
       else {
         if (c1.repr[i].n<c2.repr[i].n) return -1;
         else if (c1.repr[i].n>c2.repr[i].n) return 1;
       }
     }
     return 1;
   }
