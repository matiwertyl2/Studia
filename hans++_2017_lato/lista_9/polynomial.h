...
template <typename M, typename N> polynomial<N> operator + (polynomial<N> p1, polynomial<M> p2) {
  polynomial<N> r= p1;
  r+=p2;
  return r;
}
...
template <typename M, typename N> polynomial<N> operator * (polynomial<N> p1, polynomial<M> p2) {
  polynomial<N> r;
  for (const auto& product1 : p1.repr) {
    for (const auto& product2 : p2.repr){
      r.repr[product1.first*product2.first]+=product1.second*product2.second;
    }
  }
  return r;
}
